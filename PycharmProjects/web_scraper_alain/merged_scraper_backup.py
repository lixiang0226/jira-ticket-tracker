...
def extract_privacy_policy_details(driver):
    ...
    try:
        ...
        # DEBUG: Print the text content being searched
        logging.debug(f"Privacy Policy page text:\n{text[:1500]}")  # limit to 1500 chars
        ...
        # Date
        date_match = re.search(r"(?:last updated|effective date)[:\s]*([A-Za-z]+\s\d{1,2},\s\d{4}|\d{4}-\d{2}-\d{2})", text, re.IGNORECASE)
...
def extract_terms_details(driver):
    ...
    :return: tuple (date_found, terms_link_url) or ("Not Found", "Not Found") or ("Error", "Error")
    """
    try:
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        links = soup.find_all('a', href=True)
        ...
        for link in links:
            if any(keyword in link.get_text().lower() for keyword in terms_keywords):
                terms_link = requests.compat.urljoin(driver.current_url, link['href'])
                break

        if not terms_link:
            logging.info("T&C link not found on the page.")
            return "Not Found", "Not Found"

        logging.info(f"Found potential T&C link: {terms_link}")

        response = requests.get(terms_link, timeout=10)
        #nresponse.raise_for_status()
        #ntext = response.text

        driver.get(terms_link)
        text = driver.page_source


# DEBUG: Print the text content being searched
        logging.debug(f"Terms & Conditions page text:\n{text[:1500]}")  # limit to 1500 chars


    # Use improved regex for loose matches
        date_match = re.search(
            r"(?:effective|revised|updated).{0,200}?(?P<date>[A-Za-z]+\s\d{1,2},\s\d{4}|[A-Za-z]+\s\d{4}|\d{4}-\d{2}-\d{2})",
            text,
            re.IGNORECASE | re.DOTALL
        )
        date = date_match.group("date") if date_match else "Not Found"
        return date, terms_link

    except requests.exceptions.RequestException as e:
        logging.error(f"HTTP request error for T&C link {terms_link if 'terms_link' in locals() else 'Unknown'}: {e}")
        return "Error", (terms_link if 'terms_link' in locals() and terms_link else "Error")
    except Exception as e:
        logging.error(f"Error extracting T&C details: {e}")
        # If terms_link was found before the error, return it, otherwise "Error"
        return "Error", (terms_link if 'terms_link' in locals() and terms_link else "Error")
